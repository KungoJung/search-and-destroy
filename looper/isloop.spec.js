...
const repeatedNumbers = [1, 2, 3, 4, 5, 6, 7, 8, 9, 6, 10];
const backwardsRepeatedNumbers = [1, 2, 3, 4, 5, 6, 7, 8, 9, 6, 10];
...
test4.addToHead(0);
repeatedNumbers.forEach((val) => test4.addToTail(val));

const test5 = new LinkedList();
test5.addToHead(0);
backwardsRepeatedNumbers.forEach((val) => test5.addToTail(val));
test5.getNthNode(2).previous = test5.getNthNode(9);
...
describe("isLoop function", () => {
  ...
  it("returns true if linked list contains a loop", () => {
    ...
    expect(isLoop(test5)).to.equal(true);
  });
